...
public class BMPMultiPLC extends UFPLCifce implements Irrigator {
    ...
    /** constructors */
    public BMPMultiPLC( String host, BMPMultiPLCConfig config) {
      _plcHost = host;
      _pwrReg = config.powerRegister;
      _timerReg = config.timerRegister;
      _timerReg2 = config.timerRegister + 1;
      _timerFlagReg = config.timerFlagRegister;
      _yReg = config.outputRegister;
      _timerStatReg = config.timerStatRegister;
      _timerValReg = config.timerValRegister;
      _timerValReg2 = config.timerValRegister+1;
      _outlet = config.outletNumber;
    ...
    public BMPMultiPLC( String host, int uid ) {
      _plcHost = host;
      _uid = uid;
      _writeReq = new WriteSingleRegisterRequest(); //To be used for sending write requests to PLC.
      _readReq = new ReadInputRegistersRequest();   //Ditto for read requests.
    ...
    public static void main(String[] args) {
      //BMPMultiPLCConfig config = new BMPMultiPLCConfig(1088, 1);
      //BMPMultiPLCConfig config = new BMPMultiPLCConfig(1128, 6);
      int pwrReg = 1088, outlet = 1;
      for (int j = 0; j < args.length; j++) {
	if (args[j].equals("-reg")) pwrReg = Integer.parseInt(args[j+1]); 
	if (args[j].equals("-outlet")) outlet = Integer.parseInt(args[j+1]);
      }
      BMPMultiPLCConfig config = new BMPMultiPLCConfig(pwrReg, outlet);
      BMPMultiPLC plc = new BMPMultiPLC("192.168.1.107", config);
      plc.connect();
      //plc.verbose(true);
      for (int j = 0; j < args.length; j++) {
	if (args[j].equals("-on")) plc.powerOn();
	if (args[j].equals("-off")) plc.powerOff();
