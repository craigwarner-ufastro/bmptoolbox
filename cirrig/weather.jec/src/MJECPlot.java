...
public class MJECPlot extends UFHelperPlot {
    ...
    public MJECPlot(WthjecFrame wthjecFrame, String[] recNames) {
	wthjec = wthjecFrame;
	recsToPlot = recNames;
    ...
    public class MJECGraphThread extends UFHelperPlot.GraphThread {
	  ...
	  while (!hasData) {
	    ...
	    guiRec = (UFGUIRecord)wthjec.database.get(recsToPlot[0]);
	    ...
	    for (int j = 0; j < numSensors; j++) {
	      try {
                guiRec = (UFGUIRecord)wthjec.database.get(recsToPlot[j]);
