package javaMMTLib;
/**
 * Title:        UFMMTIntegratedTextField.java
 * Version:      (see rcsID)
 * Copyright:    Copyright (c) 2010
 * Author:       Craig Warner
 * Company:      University of Florida
 * Description:  Extends JTextField
 */
...
public class UFMMTIntegratedTextField extends JTextField implements UFMMTComponent, FocusListener, KeyListener, MouseListener {
    public static final String rcsID = "$Name:  $ $Id: UFMMTIntegratedTextField.java,v 1.7 2014/10/28 21:11:31 warner Exp $";
    ...
    public UFMMTIntegratedTextField(String name) {
      recName = name;
    ...
    public void addShadow(UFMMTComponent shadow) {
      if (shadow instanceof UFMMTIntegratedTextField) {
	shadows.add(shadow);
      }
    }
    ...
    public void setValue(String value) {
      ...
      if (unitFactor != 1) {
        try {
          ...
          outVal = String.valueOf(UFLabel.truncFormat((unitFactor*tempd),1,5));
        } catch(Exception e) {
	  System.out.println("UFMMTIntegratedTextField::setValue> ERROR: "+e.toString());
	  outVal = value.trim();
	}
      }
      prev_text = outVal; 
    ...
    public void registerComponent(UFGUIRecord guiRec, boolean isInput) {
      //UFMMTIntegratedTextField is usually both input and output
      if (isInput) {
	...
	UFMMTIntegratedTextField shadow = (UFMMTIntegratedTextField)shadows.get(j);
	shadow.clear();
        shadow.setText(getText().trim());
      }
    ...
    public String getCommandValue() {
      if (unitFactor != 1) {
        try {
          double tempd = Double.parseDouble(getText());
	  String retVal = command+UFLabel.truncFormat((tempd/unitFactor),1,5);
	  return retVal;
        } catch(Exception e) {
	  System.out.println("UFMMTIntegratedTextField> ERROR: "+e.toString());
	}
      }
    ...
    public void updateFactor(double newFactor) {
      try {
        double tempd = Double.parseDouble(this.getText());
        if (guiRec != null && !isBlue()) {
	  tempd = Double.parseDouble(guiRec.getValue());
	  setText(UFLabel.truncFormat((tempd*newFactor),1,5)+"   ");
 	} else setText(UFLabel.truncFormat((tempd*newFactor/unitFactor),1,5)+"   ");
      } catch(Exception e) {}
...
} //end of class UFMMTIntegratedTextField
