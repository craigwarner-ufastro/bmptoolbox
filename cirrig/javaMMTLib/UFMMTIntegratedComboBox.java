package javaMMTLib;
/**
 * Title:        UFMMTIntegratedComboBox.java
 * Version:      (see rcsID)
...
public class UFMMTIntegratedComboBox extends JComboBox implements UFMMTComponent { 
    public static final String rcsID = "$Name:  $ $Id: UFMMTIntegratedComboBox.java,v 1.9 2014/02/25 22:55:11 warner Exp $";
    ...
    UFGUIRecord guiRec;
    public static final Color GREEN_COLOR = new Color(128,255,128);

    String currSelected = null;
    boolean _popupCanceled = false;

    public UFMMTIntegratedComboBox(String name, String[] items, String[] refVals, int bhv) {
      recName = name;
      timeStamp = 0;
    ...
    public UFMMTIntegratedComboBox(String name, String[] items, String[] refVals, int bhv, String selected) {
    ...
    public UFMMTIntegratedComboBox(String name, String[] items, String[] refVals, int bhv, int selected) {
      this(name, items, refVals, bhv);
    ...
    public UFMMTIntegratedComboBox(String name, String[] items, int behavior) {
      this(name, items, null, behavior);
    }

    public UFMMTIntegratedComboBox(String name, String[] items, int behavior, String selected) {
      this(name, items, null, behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, String[] items, int behavior, int selected) {
      this(name, items, null, behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, String[] items) {
      this(name, items, null, ITEM);
    }

    public UFMMTIntegratedComboBox(String name, String[] items, String selected) {
      this(name, items, null, ITEM, selected);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, Vector<String> refVals, int behavior) {
    ...
    public UFMMTIntegratedComboBox(String name, Vector<String> items, Vector<String> refVals, int behavior, String selected) {
      this(name, (String[])(items.toArray()), (String[])(refVals.toArray()), behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, Vector<String> refVals, int behavior, int selected) {
      this(name, (String[])(items.toArray()), (String[])(refVals.toArray()), behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, int behavior) {
      this(name, (String[])(items.toArray()), behavior);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, int behavior, String selected) {
      this(name, (String[])(items.toArray()), behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, int behavior, int selected) {
      this(name, (String[])(items.toArray()), behavior, selected);
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items) {
      this(name, (String[])(items.toArray()));
    }

    public UFMMTIntegratedComboBox(String name, Vector<String> items, String selected) {
...
*/
      ...
      setActionCommand(ac);
      setValueWithoutMarking(guiRec.getValue());
    }
...
//-------------------------------------------------------------------------------
    ...
    public void addNewItem(String item) {
      ...
      for (int j = 0; j < shadows.size(); j++) {
        UFMMTIntegratedComboBox shadow = (UFMMTIntegratedComboBox)shadows.get(j);
...
//-------------------------------------------------------------------------------
    ...
    public void addShadow(UFMMTComponent shadow) {
      if (shadow instanceof UFMMTIntegratedComboBox) {
        shadows.add(shadow);
      }
    }
	...
	try {
	  ...
	  System.out.println("UFMMTIntegratedComboBox::setValue("+recName+")> Illegal index: "+value);
	  setBackground(Color.RED);
	}
      } else if (behavior == ITEM) {
	setSelectedItem(value);
	if (!((String)getSelectedItem()).equals(value)) {
          System.out.println("UFMMTIntegratedComboBox::setValue("+recName+")> Illegal item: "+value);
          setBackground(Color.RED);
	} else {
	  prevVal = value;
	  outVal = value;
	...
	if (idx == -1) {
          System.out.println("UFMMTIntegratedComboBox::setValue("+recName+")> Illegal value: "+value);
          setBackground(Color.RED);
	} else {
    ...
    public void setValueWithoutMarking(String value) {
      String ac = getActionCommand();
      setActionCommand("shadowing");
      setValue(value);
      setActionCommand(ac);
    ...
    public void registerComponent(UFGUIRecord guiRec, boolean isInput) {
      //UFMMTIntegratedComboBox is usually both input and output
      if (isInput) {
    ...
    public void clear() {
      ...
      if (guiRec != null) setValue(guiRec.getValue());
      marked = false; //set marked = false here after resetting value
    }
...
} //end of class UFMMTIntegratedComboBox
