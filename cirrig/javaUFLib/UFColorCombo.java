...
//Title:        UFColorCombo.java
//Copyright:    (c) 2007
//Author:       Craig Warner
//Company:      University of Florida
//Description:  Extension of JComboBox class for extra colors
...
public class UFColorCombo extends JComboBox {
    public static final
        String rcsID = "$Name:  $ $Id: UFColorCombo.java,v 1.1 2010/03/10 21:33:49 warner Exp $";
    ...
    public UFColorCombo() { super();}
    public UFColorCombo(int colorScheme) {
	super();
	setColorGradient(colorScheme);
    }
    public UFColorCombo(Object[] items) {
	super(items);
    }
    public UFColorCombo(Object[] items, int colorScheme) {
        super(items);
	setColorGradient(colorScheme);
    }
    public UFColorCombo(Vector<?> items) {
        super(items);
    }
    public UFColorCombo(Vector<?> items, int colorScheme) {
        super(items);
        setColorGradient(colorScheme);
    }
    public UFColorCombo(ComboBoxModel aModel) {
	super(aModel);
    }
    public UFColorCombo(ComboBoxModel aModel, int colorScheme) {
        super(aModel);
        setColorGradient(colorScheme);
    }
    ...
    public void setBackground(Color bg, boolean ug) {
        super.setBackground(bg);
        useGradient = ug;
    } 
	...
	useGradient = true;
	    ...
	    setColorGradient(new Color(190, 210, 225),
			     new Color(180, 200, 235),
			     new Color(135, 185, 255));
	...
	super.paintComponent(g);
	    ...
	    gp1 = new GradientPaint(1.0f, (float)getHeight(),gradientColor1, 1.0f, (float)getHeight() * 0.3f, gradientColor3);
            gp2 = new GradientPaint(1.0f, 0.0f, gradientColor2, 1.0f, (float)getHeight() * 0.3f, gradientColor3);
            rf1 = new Rectangle2D.Float(0.0f, (float)getHeight() * 0.3f, (float)getWidth(), (float)getHeight());
            rf2 = new Rectangle2D.Float(0.0f, 0.0f, (float)getWidth(), (float)getHeight() * 0.3f);
	    g2.setPaint(gp1);
            g2.fill(rf1);
            g2.setPaint(gp2);
            g2.fill(rf2);
	    ...
	    //paint border
            g2.setPaint(new Color(75,75,150));
		...
		g2.drawString(strs[i], 4, (int)(getHeight()/2 + r2d.getHeight()/2*strs.length - (r2d.getHeight())*(strs.length-1-i) ));
	    }
	} else {
	    g2.drawString (text, 4, (int)(getHeight()/2 +  r2d.getHeight()/2));
	}
    }
	...
	JFrame j = new JFrame("UFColorCombo Test");
	j.setDefaultCloseOperation(3);
	j.setSize(400,400);
	JPanel pan = new JPanel();
	final UFColorCombo ufcb = new UFColorCombo("test1,test --- long test -- 2,test3".split(","),COLOR_SCHEME_BLUE);
	ufcb.setBackground(Color.ORANGE, true);
	pan.add(ufcb);
