...
class chart {
  ...
  var $overrideXticks = false;
  var $xtickvalues = array(false);
  var $xticknames = array(false);
  var $xtickinterval = 0;
  var $overrideYticks = false;
  var $ytickvalues = array(false);
  var $yticknames = array(false);
  var $ytickinterval = 0;

  function set_xtickinterval($xtickinterval) {
    $this->xtickinterval = $xtickinterval;
  }

  function set_xtickv($xtickvalues) {
    $this->xtickvalues = $xtickvalues;
    $this->overrideXticks = true;
  }

  function set_xticknames($xticknames) {
    $this->xticknames = $xticknames;
  }

  function set_ytickinterval($ytickinterval) {
    $this->ytickinterval = $ytickinterval;
  }

  function set_ytickv($ytickvalues) {
    $this->ytickvalues = $ytickvalues;
    $this->overrideYticks = true;
  }

  function set_yticknames($yticknames) {
    $this->yticknames = $yticknames;
  }
  ...
  function set_extrema ($y_min = array(), $y_max = array(),
    ...
    if (! is_array($x_max))
      $this->x_max = $x_max;
    $this->x_label = "TEST $y_min $x_min $y_max $x_max";
  }
  ...
  function stroke ($callback = false) {
    ...
    $title_color = $this->allocate_color($this->title_color[0]);
...
#      return substr($monthnames[date("m", $time)-1], 0, 3) . " " . date("Y", $time);
  ...
  function draw_x_axis ($im, $xmin, $xmax, $xs, $ys, $width,
	...
	$ticks = $this->get_ticks($xmin, $xmax, $ys, $this->tick_whole_x, "X");
      }
	...
	if ($do_text) {
	  if (((($i+$number_unsuitable) % $step) == 0 and !$this->overrideXticks) or ($this->overrideXticks and array_search($x, $this->xtickvalues) !== false)) {
	    if ($this->overrideXticks) {
	      $text = $this->xticknames[array_search($x, $this->xtickvalues)];
	    } elseif ($do_tick_texts) {
	      $text = $tick_texts[$i];
	    } elseif ($this->x_ticks) {
...
fclose($f);
  ...
  function draw_y_axis ($im, $ymin, $ymax, $xs, $ys,
	...
	list($valfactor) = $this->pleasing_numbers(($ymax-$ymin)/1000, $series);
      }
      if ($this->overrideYticks) {
        $ticks = $this->get_ticks($ymin, $ymax, $height, $whole=true);
	$length = sizeof($ticks);
	$factor = 1;
      }
	  ...
	  if ((! ((int)($iy*100)%($factor*100))) && ($iy != -1)) {
	    ...
	    if (($y*10)%10)
	      $whole = false;
            ...
            if ($this->overrideYticks) {
	      if (array_search($y, $this->ytickvalues) !== false) {
                $yst = $this->yticknames[array_search($y, $this->ytickvalues)];
	      } else $yst = ""; 
	    }
  ...
  function get_ticks ($min, $max, $height, $whole = false, $axis="Y") {
    $diff = abs($max-$min);
    ...
    if ($this->xtickinterval > 0 and $axis == "X") $even = $this->xtickinterval;
    if ($this->ytickinterval > 0 and $axis == "Y") $even = $this->ytickinterval;

    if ($min < 0) 
      $start = floor($min*1000) + $even*1000-(floor($min*1000)%($even*1000))
	- $even*1000;
    elseif ($min == 0)
      $start = 0;
    else {
    ...
    for ($elem = $start, $i = 0;
	 $elem < $max*1000; $elem += (int) floor($even*1000), $i++) {
      $ticks[$i] = $elem/1000;
      if ($i > 1000)
	return $ticks;
    }
...
class plot {
  ...
  function get_extrema ($dim, $force_true = false) {
    ...
    if ($dim > $this->dimension ||
        (!$force_true && $dim == 1 && ($this->style == "hbar")))
      return array(0, sizeof($this->coords[1])-1);
  ...
  function stroke ($im, $xmin, $xmax, $ymin, $ymax, $xoff, $yoff,
	...
	elseif ($style == 5) {
	  ...
	  // Fill
        ...
        #imagestring($im, $this->font, 80, $i*25+25, "$i $yt $oyt $pxt $xt" , $color);
        #imagestring($im, $this->font, 80, $i*25+25, "$i " . ($this->coords[1][$i]*1.0), $color);
	    for ($j = $pxt; $j <= $xt; $j++) 
	    ...
	    $poyt = $oyt;
          ...
          // Fill horizontal bars
            if (! isset($poxt))
              $poxt = $oxt;
	#imagestring($im,  $this->font, 100, 100, "$end", $color);
	#imagestring($im, $this->font, 100, $i*25+50, "$i  " . ($this->coords[0][$i]*1.0) . " $xmin $xmax A", $color);
        #imagestring($im, $this->font, $xt, $yt, "$xmin $ymin $xmax $ymax",
                    #$color);
	    $oxt = $xoff + #$width - 
              ((0.0 - $xmin) / ($xmax - $xmin) * $width);
	    #$xt = $xoff + #$width
		#(($x*1.0 - $xmin) / ($xmax - $xmin) * $width);
            #$oyt = $yoff + $height -
              #(($this->coords[1][$i]*1.0 - $ymin) / ($ymax - $ymin) * $height);
        #imagestring($im, $this->font, 100, $i*25+50, "$i $xt $oxt $pyt $yt $x $y" , $color);
            for ($j = $yt; $j <= $pyt; $j++)
	      imageline($im, $oxt, $j, $xt, $j, $color);
            $poxt = $oxt;
	} elseif ($style == 7) {
