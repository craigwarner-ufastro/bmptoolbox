...
<script language="javascript">
  ...
  function checkWS() {
	...
	a = false;
        ...
        a = false;
      } else if (document.manageWS.et_fac.value == '') {
	alert('You must enter a value for et_fac');
	a = false;
      }
    }
    return a;
  ...
  function editWS(n) {
    ...
    var td_et_fac = document.getElementById("td_et_fac"+n);
    ws_et_fac[n] = td_et_fac.innerHTML;
    ...
    td_et_fac.innerHTML = '<input type="text" name="et_fac_' + n + '" size=2 value="' + ws_et_fac[n]+'" onkeypress="return validDec(event);">';
  }
  ...
  function cancelEdit(n) {
    ...
    td_et_fac.innerHTML = ws_et_fac[n];
  }
</script>
<?php
...
<?php 
   if ($mode == 'changepass' && $isLoggedIn) {
    ...
    foreach ($_POST as $key=>$value) {
      ...
      } else if (strpos($key, 'et_fac_') !== false) {
	$wsid = substr($key, strrpos($key, '_')+1);
        $sql = "update weatherStations set et_fac=$value where wsid=$wsid and uid=" . $_SESSION['bmpid'] . ";";
    ...
    $et_fac = trim($_POST['et_fac']);
    $wsTime = trim($_POST['weatherTime']);
    if ($wsLocation != '' && $elev != '' && $lattitude != '' && $longitude != '') {
      #Add new weather station 
      $sql = "insert into weatherStations(uid, location, elevation_ft, lattitude, longitude, et_fac, weatherTime) values (" . $_SESSION['bmpid'] . ", '$wsLocation', $elev, $lattitude, $longitude, $et_fac, '$wsTime');";
      $out = $mysqli_db->query($sql);
      if ($out == false) {
...
?>
      ...
      <th valign="top" class="manageth">ET<br>Fac</th>
...
<?php
	...
	echo $td . ' id="td_et_fac' . $info[$j]['wsid'] . '">' . $info[$j]['et_fac'] . '</td>';
...
?>
      ...
      <td><input type="text" name="et_fac" size=2 onkeypress="return validDec(event);"></td>
