...
<script language="javascript">
  ...
  function changeContSize(contSize) {
    ...
    //Update cost basis menu
    if (pagename == "realtime") return;
    document.paramForm.costBasis.options.length=0;
    if (contSize == 1) {
      for (j = 0; j < costNamesT1.length; j++) {
        document.paramForm.costBasis.options[j] = new Option(costNamesT1[j], costIdsT1[j], false, false);
      }
    } else if (contSize == 3) {
      for (j = 0; j < costNamesT3.length; j++) {
        document.paramForm.costBasis.options[j] = new Option(costNamesT3[j], costIdsT3[j], false, false);
      }
    }
  }
...
<?php
   ...
   $wthX = array(254, 216, 216, 192, 209, 29, 202, 115, 254);
   $wthY = array(168, 96, 165, 54, 27, 27, 116, 23, 192);
   #$wthLat = array("26.650", "28.450", "26.583", "29.700", "30.500", "30.783", "28.017", "30.600", "25.767");
   #$wthLong = array("-80.633", "-81.750", "-81.867", "-82.283", "-81.700", "-87.133", "-82.150", "-84.550", "-80.817");
   #$wthElev = array("5", "34", "5", "38", "8", "66", "37", "75", "5");
   $wthStart = "Jan 1, 1968";
   $wthLat = array();
   $wthLong = array();
   $wthElev = array();
   ...
   for ($j = 0; $j < count($wth); $j++) {
      $info = getWthInfo($wth[$j] . '.wth');
      $wthLat[] = $info["lat"];
      $wthLong[] = $info["long"];
      $wthElev[] = $info["elev"];
      $wthEnd[] = getWthEndDate($wth[$j] . '.wth');
   }
   ...
   while ($temp = readdir($f)) {
      ...
      #if (eregi('.wth', $temp)) {
        $fawn[] = 'fawn/' . substr($temp, 0, strpos($temp, '.wth'));
      }
   }
   sort($fawn);
   $fawnStart = array();
   ...
   $fawnId = array();
   $fawnElev = array();
   $temp = file("fawn/FAWN_STATIONS.csv");
   for ($j = 0; $j < count($fawn); $j++) {
      $loc = str_replace('_',' ',substr($fawn[$j], 5));
      for ($l = 0; $l < count($temp); $l++) {
	if (strpos($temp[$l], $loc) !== false) {
	  $data = explode(",", $temp[$l]);
	  $last = count($data)-1;
	  $spos = strpos($data[4], ' ');
	  $fawnStart[] = substr($data[4], $spos+1) . ', ' . substr($data[4], 0, $spos);
	  $fawnId[] = $data[1];
	  $fawnLat[] = $data[2];
	  $fawnLong[] = $data[3];
	  $fawnElev[] = trim($data[count($data)-1]);
	}
      }
   }
   #Pass arrays and vars to javascript
   echo '<script language="javascript">';
   for ($j = 0; $j < count($wth); $j++) {
      echo 'wth[' . $j . ']="' . $wth[$j] . '";' . "\n";
      echo 'wthLat[' . $j . ']="' . $wthLat[$j] . '";' . "\n";
      echo 'wthLong[' . $j . ']="' . $wthLong[$j] . '";' . "\n";
      echo 'wthElev[' . $j . ']="' . $wthElev[$j] . '";' . "\n";
      echo 'wthEnd[' . $j . ']="' . $wthEnd[$j] . '";' . "\n";
   }
   echo 'var wthStart="' . $wthStart . '";';
   for ($j = 0; $j < count($fawn); $j++) {
      echo 'fawn[' . $j . ']="' . $fawn[$j] . '";' . "\n";
      echo 'fawnId[' . $j . ']="' . $fawnId[$j] . '";' . "\n";
      echo 'fawnLat[' . $j . ']="' . $fawnLat[$j] . '";' . "\n";
      echo 'fawnLong[' . $j . ']="' . $fawnLong[$j] . '";' . "\n";
      echo 'fawnStart[' . $j . ']="' . $fawnStart[$j] . '";' . "\n";
      echo 'fawnElev[' . $j . ']="' . $fawnElev[$j] . '";' . "\n";
   }
   echo 'var latest="' . $latest . '";';
   echo '</script>';
   ...
   if ($isLoggedIn) {
	  ...
	  if ($row['type'] == 'wth') {
	    $userWth[] = "data/" . $bmpusername . "/" . substr($row['name'], 0, strpos($row['name'], '.wth')); 
	  } else if ($row['type'] == 'plt') {
	    $userPlt[] = "data/" . $bmpusername . "/" . substr($row['name'], 0, strpos($row['name'], '.plt'));
	  } else if ($row['type'] == 'irr') {
            $userIrr[] = "data/" . $bmpusername . "/" . substr($row['name'], 0, strpos($row['name'], '.irr'));
	  } else if ($row['type'] == 'sfn') {
            $userSfn[] = "data/" . $bmpusername . "/" . substr($row['name'], 0, strpos($row['name'], '.sfn'));
	  }
	}
      }
      sort($userWth);
	...
	$info = getWthInfo($userWth[$j] . '.wth');
        $userWthStart[] = $info["start"];
	$userWthLat[] = $info["lat"];
	$userWthLong[] = $info["long"];
	$userWthElev[] = $info["elev"];
	$userWthEnd[] = getWthEndDate($userWth[$j] . '.wth');
      }
      #Pass arrays and vars to javascript
      echo '<script language="javascript">';
      for ($j = 0; $j < count($userWth); $j++) {
	echo 'userWth[' . $j . ']="' . $userWth[$j] . '";' . "\n";
        echo 'userWthStart[' . $j . ']="' . $userWthStart[$j] . '";' . "\n";
        echo 'userWthLat[' . $j . ']="' . $userWthLat[$j] . '";' . "\n";
        echo 'userWthLong[' . $j . ']="' . $userWthLong[$j] . '";' . "\n";
        echo 'userWthElev[' . $j . ']="' . $userWthElev[$j] . '";' . "\n";
        echo 'userWthEnd[' . $j . ']="' . $userWthEnd[$j] . '";' . "\n";
      } 
      echo '</script>';
   }
...
<table align="center" cellspacing=0 cellpadding=0 class="maintable">
   ...
   <td valign="top" style="position:relative;">
	...
	<tr>
	<td valign="top" align="left"><a href="driver_tech.php" onMouseOver="on('technical');" onMouseOut="off('technical');"><img src="images/button-off-technical.png" border=0 name="technical" alt="Technical"></a></td>
	</tr>
<!--
        <tr>
